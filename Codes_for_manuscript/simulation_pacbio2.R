...
start_time <- Sys.time()
# read S
Y = read.table("./result_simulated_2/Indexd.csv", header=T, sep=',')
method = Y[,1]
Y = t(Y[,-1])

# read S
S = read.table("./result_simulated_2/Scored.csv", header=T, sep=',')
S = t(S[,-1])

# re-order
reset = sapply(c("cutesv", "pbsv", "sniffles", "svim"), function(c) return(which(method==c)))
Y = Y[,reset]
S = S[,reset]
method = method[reset]
...
a.censor = c(-Inf, -Inf, -Inf, 0)
b.censor = c(Inf, Inf, 60, Inf)
...
info = read.table("./result_simulated_2/IndexSVInfo.csv", header=T, sep=',')
...
merged = read.table(paste(path, "merged_2.csv", sep = ''), header=T, sep=',')
method = c("cutesv", "pbsv", "sniffles", "svim")
...
row_names <- c("cuteSV","pbsv","Sniffles","SVIM","Model-0.950","Model-0.990","Model-0.999","combiSV","Vote 1","Vote 2")
...
write.csv(data, file = "table5.csv")
