...
set.seed(123)
start_time <- Sys.time()
Y = read.table("./result_NA19238/Indexd.csv", header=T, sep=',')
method = Y[,1]
Y = t(Y[,-1])

# read S
S = read.table("./result_NA19238/Scored.csv", header=T, sep=',')
S = t(S[,-1])
...
a.censor = c(-Inf, -Inf, -Inf, -Inf, min(S[,5], na.rm=T))
b.censor = c(Inf, Inf, max(S[,3], na.rm=T), max(S[,4], na.rm=T), Inf)

mu_prior = c(NA, NA, 60, 60, 0)
mu_prior = c(NA, NA, max(S[,3], na.rm=T), max(S[,4], na.rm=T), min(S[,5], na.rm=T))
sigma_prior = c(NA, NA, 0.01, 0.01, 1)
...
info = read.table("./result_NA19238/IndexSVInfo.csv", header=T, sep=',')
...
write.csv(res_D_0.95, './result_NA19238/res_D_0.95.csv', row.names = F)

D_0.99 = P_esi>x[which.min(abs(y-0.01))]
res_D_0.99 = info[D_0.99,]
write.csv(res_D_0.99, './result_NA19238/res_D_0.99.csv', row.names = F)

D_0.999 = P_esi>x[which.min(abs(y-0.001))]
res_D_0.999 = info[D_0.999,]
write.csv(res_D_0.999, './result_NA19238/res_D_0.999.csv', row.names = F)
...
write.csv(res_D_vote1, './result_NA19238/res_D_vote1.csv', row.names = F)
write.csv(res_D_vote2, './result_NA19238/res_D_vote2.csv', row.names = F)

cat("\n", "Estimated FDR\n")
cat(1-FDR1, mean(P_esi[D_vote1]), mean(P_esi[D_vote2]))

cat('\n', 'Number of SVs\n')
cat(colSums(Y), sum(D_0.95), sum(D_0.99), sum(D_0.999), sum(D_vote1), sum(D_vote2))
cat('\n')

fdr_na19238 <- c(1-FDR1, 0.95,0.99,0.999,mean(P_esi[D_vote1]), mean(P_esi[D_vote2]))
nsv_na19238 <- c(colSums(Y), sum(D_0.95), sum(D_0.99), sum(D_0.999), sum(D_vote1), sum(D_vote2))

row_names <- c("cuteSV","pbsv","Sniffles","DeBreak","SVIM","Model-0.950","Model-0.990","Model-0.999","Vote 1","Vote 2")
col_names <- c("number of SVs","1-FDR","number of SVs","1-FDR")  

simulation1_results <- cbind(nsv_na19238,fdr_na19238)

set.seed(123)
Y = read.table("./result_NA19239/Indexd.csv", header=T, sep=',')
method = Y[,1]
Y = t(Y[,-1])

# read S
S = read.table("./result_NA19239/Scored.csv", header=T, sep=',')
S = t(S[,-1])
...
info = read.table("./result_NA19239/IndexSVInfo.csv", header=T, sep=',')
...
D_0.95 = P_esi>x[which.min(abs(y-0.05))]
res_D_0.95 = info[D_0.95,]
write.csv(res_D_0.95, './result_NA19239/res_D_0.95.csv', row.names = F)

D_0.99 = P_esi>x[which.min(abs(y-0.01))]
res_D_0.99 = info[D_0.99,]
write.csv(res_D_0.99, './result_NA19239/res_D_0.99.csv', row.names = F)

D_0.999 = P_esi>x[which.min(abs(y-0.001))]
res_D_0.999 = info[D_0.999,]
write.csv(res_D_0.999, './result_NA19239/res_D_0.999.csv', row.names = F)
...
write.csv(res_D_vote1, './result_NA19239/res_D_vote1.csv', row.names = F)
write.csv(res_D_vote2, './result_NA19239/res_D_vote2.csv', row.names = F)
...
fdr_na19239 <- c(1-FDR1, 0.95,0.99,0.999,mean(P_esi[D_vote1]), mean(P_esi[D_vote2]))
nsv_na19239 <- c(colSums(Y), sum(D_0.95), sum(D_0.99), sum(D_0.999), sum(D_vote1), sum(D_vote2))


simulation_results <- NULL
simulation_results <- cbind(nsv_na19238,fdr_na19238,nsv_na19239,fdr_na19239)

data <- data.frame(simulation_results)
colnames(data) <- col_names  
rownames(data) <- row_names

write.csv(data, file = "table7.csv")
end_time <- Sys.time()
end_time-start_time