...
start_time <- Sys.time()
# read S
Y = read.table("./result_HG002/Indexd.csv", header=T, sep=',')
method = Y[,1]
Y = t(Y[,-1])

# read S
S = read.table("./result_HG002/Scored.csv", header=T, sep=',')
S = t(S[,-1])
...
info = read.table("./result_HG002/IndexSVInfo.csv", header=T, sep=',')
...
# the value
rr_combisv = idx(info, combisv, Span_loc =500, Ratio_len = 2)

D_vote1 = apply(Y, 1, function(x) sum(x)>J/2)
D_vote2 = apply(Y, 1, function(x) sum(x)==J)
fdr_hg002 <- c(1-FDR1, 0.95,0.99,0.999, mean(P_esi[rr_combisv[[1]]==1]),mean(P_esi[D_vote1]), mean(P_esi[D_vote2]))


data <- data.frame(fdr_hg002)
row_names <- c("cuteSV","pbsv","Sniffles","Debreak","SVIM","Model-0.950","Model-0.990","Model-0.999","combiSV","Vote 1","Vote 2")
col_names <- "1-FDR"
colnames(data) <- col_names  
rownames(data) <- row_names

write.csv(data, file = "table6.csv")
end_time <- Sys.time()
end_time-start_time
